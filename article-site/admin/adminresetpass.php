...
if($conn){
$query="SELECT username,password FROM admin_info WHERE username='$username' && password='$oldpassword' ";
$result=mysqli_query($conn,$query);
if(mysqli_num_rows($result)>0){
    $password=$_POST['password'];
    $password=mysqli_real_escape_string($conn,$password);
    $password=crypt($password,$salt.$hash);
    $query="UPDATE admin_info SET password='$password' WHERE username='$username' && password='$oldpassword'";
    $result=mysqli_query($conn,$query);
