...
<form class="text-center border border-light p-5" name="reg" method="POST" onsubmit="return validation();">
    ...
    <!-- Terms of service -->
<p>Email Not accessable?
        <a href="http://127.0.0.1/myproject/registration.php">Sign Up With New Account</a>
</p>
...
$query="SELECT id,username from form_info WHERE email='$email'";
$result=mysqli_query($conn,$query);
if(mysqli_num_rows($result)>0){
$token='bheuljn7r512kj90asm56532gkahujd';
$token=str_shuffle($token);
$token=substr($token,0,10);
$query="UPDATE form_info SET token='$token',tokenexpire=DATE_ADD(NOW(),INTERVAL 5 MINUTE) WHERE email='$email'";
...
if($res){
    ...
    $mail->setFrom("clgproject44198@gmail.com","reset user password");
    $mail->Port = 587;
    ...
    http://127.0.0.1/myproject/resetpass.php?email=$email&token=$token
