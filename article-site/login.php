...
if(isset($_COOKIE['2r3k'])){
    echo "<script>
    document.getElementById('alert').style.visibility='visible';
    document.getElementById('alert').innerText='password changed';
    </script>";
    setcookie('2r3k','',time() - 3600);
}
...
<input type="text" name="username" autocomplete="off"  class="form-control mb-4" placeholder="enter your username" value="<?php if(isset($_COOKIE['member_login'])){ echo /*openssl_decrypt($_COOKIE['member_login'], $cipher, $encryption_key, $options=0, $_COOKIE['53e0'],$_COOKIE['2e5nc']);*/base64_decode(base64_decode($_COOKIE['member_login'])); }?>" required />
        </div>
    </div>

    <!-- Password -->
    <label for="password" class="lbl">Password:</label>
    <input type="password" class="form-control mb-4" name="password" placeholder="enter your password" value="<?php if(isset($_COOKIE['E25#tc6'])){ echo openssl_decrypt($_COOKIE['E25#tc6'], $cipher, $encryption_key, $options=0, $_COOKIE['53e0'],$_COOKIE['2e5nc']); }?>" required />
...
<p>Forgot your password?
        <a href="http://127.0.0.1/myproject/forgotpass.php">Click Here</a>
    </p>
    <p>Admin Sign In
    <a href="http://127.0.0.1/myproject/admin/adminlogin.php">Click Here</a>
    </p>
...
if(!$connection->connect_error){
    ...
    setcookie ("53e0",$iv,time()+ (10 * 365 * 24 * 60 * 60));
    setcookie("member_login",base64_encode(base64_encode($_POST["username"])),time()+ (10 * 365 * 24 * 60 * 60));
    //setcookie ("member_login",openssl_encrypt($_POST["username"],$cipher,$encryption_key,$options=0,$iv,$tag),time()+ (10 * 365 * 24 * 60 * 60));
    setcookie ("E25#tc6",openssl_encrypt($_POST["password"],$cipher,$encryption_key,$options=0,$iv,$tag),time()+ (10 * 365 * 24 * 60 * 60));
    setcookie ("2e5nc",$tag,time()+ (10 * 365 * 24 * 60 * 60));
    }
    }
    $query= "SELECT username,password from  form_info
    WHERE username='$username' && password='$password'";
    if(mysqli_commit($connection)){
    $result=mysqli_query($connection,$query);
    if(mysqli_num_rows($result)>0){
    $query= "SELECT username,password from  form_info
    WHERE username='$username' && password='$password' && verifiedemail=1";
    $result=mysqli_query($connection,$query);
    if(mysqli_num_rows($result)>0){
    ...
    header("location: welcome.php?".$_SESSION['id']);
    }else{
    ...
    exit("<script>
    document.getElementById('alert').style.visibility='visible';
    document.getElementById('alert').innerText='Email not verified!!!!verify your email and try again later'; 
     </script>");
}
    }else{
        echo "<script>
