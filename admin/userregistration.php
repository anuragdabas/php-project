...
<form class="text-center border border-light p-5" method="post" name="reg" id="reg" action="<?php $_SERVER['PHP_SELF'] ?>"  onsubmit="return validation()">
    ...
    <div class="form-row mb-4">
        <div class="col">
            ...
            <input type="text" name="username" class="form-control" placeholder="enter user name" value="<?php if(isset($username)){echo $username;} ?>" required />
        </div>
    </div>

    <!-- E-mail -->
    <label for="email" class="lbl">Email:   </label>
    <input type="text" name="email" class="form-control mb-4" placeholder="enter user email" value="<?php if(isset($email)){echo $email;} ?>" required />

    <!-- Password -->
    <label for="password" class="lbl">Password:   </label>
    <input type="password" name="password" class="form-control" placeholder="enter user Password" aria-describedby="defaultRegisterFormPasswordHelpBlock" required />
    <small id="defaultRegisterFormPasswordHelpBlock" class="form-text text-muted mb-4">
        At least 6 characters
    </small>
    <!-- repass -->
    <label for="repassword" class="lbl">Re-enter Password:   </label>
    <input type="password" name="repassword" class="form-control" placeholder="again enter user password" aria-describedby="defaultRegisterFormPasswordHelpBlock" required />
    <small id="defaultRegisterFormPasswordHelpBlock" class="form-text text-muted mb-4">
        Again Enter user Password
    </small>

    <!-- Phone number -->
    <label for="mobilenumber" class="lbl">Phone Number:   </label>
    <input type="text" name="mobilenumber" pattern="[5-9]{1}[0-9]{9}" id="defaultRegisterPhonePassword" class="form-control" placeholder="enter user Phone number" aria-describedby="defaultRegisterFormPhoneHelpBlock" value="<?php if(isset($mobilenumber)){echo $mobilenumber;} ?>" required />
...
<div class="custom-control custom-checkbox">
    <input type="checkbox" class="custom-control-input" name="verifiedemail" id="verifiedemail">
...
<br />
    <!-- Sign up button -->
    <input class="btn btn-info my-4 btn-block" type="submit" id="submit" name="submit" value="Add As User" />
...
if($connection->connect_error){
    ...
    $query="INSERT INTO form_info(username,password,email,mobilenumber,gender,created,verifiedemail)
    VALUES ('$username','$password','$email','$mobilenumber','$gender',CURRENT_TIMESTAMP,'$e_ver')";
    $result=$connection->query($query);
    //$result=mysqli_query($connection,$query);
    if($result){
        die("<script>
        document.getElementById('alert').style.visibility='visible';
        document.getElementById('alert').innerText='User Added Successfully';
        </script>");
    }
    else{
